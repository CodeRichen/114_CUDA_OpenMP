...
__global__ void example(int **data) {
   ...
   value1 = *(data[idx1]);
   value2 = *(data[idx2]);
   value3 = value1 + value2;	//value3=2
   value4 = value1 * value2;	//value4=1
   value5 = value3 + value4;	//value5=3
   *(data[idx3]) = value5;		//mem[blockIdx.x * blockDim.x+threadIdx.x]=3
   *(data[idx1]) = value3;		//mem[blockIdx.x * blockDim.x]=2
   *(data[idx2]) = value4;		//mem[threadIdx.x]=1
   idx1 = idx2 = idx3 = 0;
}
