...
__global__ void vecSum_gpu_kernel_3(float vecC[]) {
    int id = blockIdx.x*blockDim.x + threadIdx.x;
    int tid = threadIdx.x;
	__shared__ float vec[THREADSPERBLOCK];
...
__global__ void vecSum_gpu_kernel(float vecC[]) {
    int id = blockIdx.x*blockDim.x + threadIdx.x;
    int tid = threadIdx.x;
    //float local_sum = 0;
...
int main (int argc, char **argv) {
    ...
    *S_gpu = *S_cpu = 0;
    ...
    cudaDeviceSynchronize();
 	//vecSum_gpu_kernel_3<<<BLOCKSPERGRID, THREADSPERBLOCK>>>(d_vecC);
	vecSum_gpu_kernel<<<BLOCKSPERGRID, THREADSPERBLOCK>>>(d_vecC);
