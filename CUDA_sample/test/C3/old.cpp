...
bool try_key(const string& plaintext, const string& target, const string& key, int n, int tgt_len, vector<vector<int>>& hit_perms) {
    ...
    // ── 計算 key ASCII 總和（不含 R1/R2）
    uint64_t key_ascii_sum = 0;
    for (char c : key) key_ascii_sum += static_cast<unsigned char>(c);

    // ── R1 seed：key ASCII + "R1"
    uint64_t seed_r1 = key_ascii_sum;
    for (char c : string("R1")) seed_r1 += static_cast<unsigned char>(c);

    // ── R2 seed：key ASCII + "R2"
    uint64_t seed_r2 = key_ascii_sum;
    for (char c : string("R2")) seed_r2 += static_cast<unsigned char>(c);
    ...
    for (const auto& perm : ALL_PERMS) {
        ...
        // ════ 第 1 輪 Huffman 加密（R1）════
        Seed256 seed_r1_obj{};
        seed_r1_obj.data[7] = seed_r1;

        string r1_output = "";
        for (char char_item : after_perm) {
            r1_output += encode_char_fast(seed_r1_obj, char_item);
            seed_r1_obj.update(char_item);
        }

        // ════ 第 2 輪 Huffman 加密（R2，輸入為 R1 的二進位字串）════
        // 將 R1 的 01 字串視為 36 字元集的字元流來再加密，
        // 但 R1 輸出只含 '0'/'1'，需先把它對映回一個合法字元流。
        // 這裡直接對 r1_output 的每個字元（'0' 或 '1'）做第二輪 Huffman。
        Seed256 seed_r2_obj{};
        seed_r2_obj.data[7] = seed_r2;

        int pos = 0;
        bool ok = true;
        for (char bit_char : r1_output) {
            string code = encode_char_fast(seed_r2_obj, bit_char);
            int len = code.length();
            if (pos + len > tgt_len || target.compare(pos, len, code) != 0) { ok = false; break; }
            pos += len;
            seed_r2_obj.update(bit_char);
        }
        if (ok && pos == tgt_len) {
...
int main() {
    ...
    string target_cipher = "0110011100011100010000111100110010010100010111111110110010010101010000101010001000001110010100001010011111111101111101100001111110111101110";
