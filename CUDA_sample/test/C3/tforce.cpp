...
// 預先產生所有 4! = 24 種排列
static std::vector<std::array<int,4>> ALL_PERMS;
                    ...
                    std::array<int,4> &found_perm)
{
    ...
    // seed_base = sum(ord(c) for c in key) + 82 + 49
    uint32_t seed_base = 82 + 49;
    for (int i = 0; i < 4; i++) seed_base += (unsigned char)key[i];

    // Step 2+3: 枚舉 24 種排列
    for (const auto &perm : ALL_PERMS) {
        char after_perm[4];
        for (int i = 0; i < n; i++)
            after_perm[i] = after_poker[perm[i]];
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (pos + code_len > tgt_len) { ok = false; break; }

            // 比對 target[pos..pos+code_len)
            // code bit 從 MSB 開始（最高位對應最早輸出的 bit）
...
// 每個 worker 負責 [start_idx, end_idx) 的 key 空間
            ...
            uint64_t start_idx, uint64_t end_idx)
{
    ...
    for (uint64_t idx = start_idx; idx < end_idx; idx++) {
        ...
        std::array<int,4> found_perm;
        if (try_key(key, plaintext, target_cipher, n, tgt_len, found_perm)) {
            std::string found_key(key, 4);
            {
                std::lock_guard<std::mutex> lk(g_result_mutex);
                g_results.push_back({ found_key, found_perm });
            }
...
int main() {
    ...
    printf("==============================================================\n");
    printf("  暴力破解（C++ 多執行緒 + 枚舉 4! 種換位）\n");
    printf("  明文    : %s\n", plaintext.c_str());
    printf("  目標密文: %s\n", target_cipher.c_str());
    printf("  Key 長度: %d 位  Key 空間: %llu 種\n",
           key_length, (unsigned long long)total);
    printf("  每 key 排列: %zu 種  總搜尋: %llu\n",
           ALL_PERMS.size(), (unsigned long long)(total * ALL_PERMS.size()));
    printf("  執行緒數: %d\n", num_threads);
    printf("==============================================================\n");
    ...
    if (g_results.empty()) {
        ...
        for (const auto &r : g_results) {
            printf("  找到 key = '%s'  排列 = [%d,%d,%d,%d]\n",
                   r.key.c_str(),
                   r.perm[0], r.perm[1], r.perm[2], r.perm[3]);
        }
    }
    printf("==============================================================\n");
    return 0;
