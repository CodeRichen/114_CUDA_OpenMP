...
int main(int argc, char **argv)
{
  ...
  checkCuda( cudaMemcpyAsync(&d_a[offset2], &a[offset2], 
                               streamBytes, cudaMemcpyHostToDevice,
                               stream[1]) );							   
  int offset3 = 3 * streamSize;
  checkCuda( cudaMemcpyAsync(&d_a[offset3], &a[offset3], 
                               streamBytes, cudaMemcpyHostToDevice,
                               stream[2]) );
  ...
  kernel<<<streamSize/blockSize, blockSize, 0, stream[2]>>>(d_a, offset3);  
  ...
  checkCuda( cudaMemcpyAsync(&a[offset2], &d_a[offset2], 
                               streamBytes, cudaMemcpyDeviceToHost,
                               stream[1]) );
  checkCuda( cudaMemcpyAsync(&a[offset3], &d_a[offset3], 
                               streamBytes, cudaMemcpyDeviceToHost,
                               stream[2]) );							   
